...
When term starts we tend to get quite a lot of laptops that fail to boot in to OS. Usually it is caused by dammaged HDD In workshop we have disk cloner, which is capable of making one-to-one copy. It becomes extremely useful since disk copier can skip broken sectors, filling them with zeros on destination drive and then it is possible to copy at least some data from working copy.
