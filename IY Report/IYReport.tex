...
\section{Interzone}
Interzone is a web based front end for database that is used to manage records of network equipment in Aberystwyth University. On the main page you can search for record that contains specified information in it. You can search by IP address, MAC address, Owner's login, computer name etc. On the figure ~\ref{fig:main_interzone_page} you can see main Interzone page.
...
Information that you can gather from interzone is very useful. It is possible to check DHCP log to see if computer gets IP address, look at RADIUS logs that contains information if the user was successfully in authenticating 
